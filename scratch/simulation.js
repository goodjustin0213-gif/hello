...
<head>
    ...
    <title>空軍官校職涯財務決策支援系統 | v6.0 戰略指揮官版</title>
        ...
        <div class="flex items-center gap-4">
            ...
            <div>
                ...
                <div class="flex items-center gap-2">
                    ...
                    <p class="text-slate-400 text-[10px] tracking-[0.3em] font-mono">STRATEGIC COMMAND v6.0</p>
                </div>
            </div>
        </div>
    ...
    <script>
        ...
        function collectList(className, valClass) {
            ...
            document.querySelectorAll('.' + className).forEach(row => {
                ...
                const val = Number(row.querySelector('.' + valClass).value) || 0; // Fix: Ensure Number
        ...
        // --- Core Simulation Logic (Fixed NaN) ---
        function calculateScenarioData(params) {
            // Defensive Copy & Default Values
            const p = {
        ...
        function renderCharts(res, compRes) {
            ...
            // Liquid Asset = Net Asset - House Value + Loan (Logic check: Liquid = Cash + Invest return)
            // But here we use calculated Liquid Asset from simulation.
            // Wait, liquidAsset in logic includes house impact? No. 
            // Let's use: House Value (Area), Liquid Assets (Area), Loan (Line)
            // Need to reconstruct Liquid from Net: Net = Liquid + House - Loan => Liquid = Net - House + Loan
